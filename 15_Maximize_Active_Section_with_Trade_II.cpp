...
public:
    vector<int> maxActiveSectionsAfterTrade(string s, vector<vector<int>>& queries) {
        ...
        auto getMaxInRange = [&](int left, int right) {
            ...
            return max(rmq[power][left],
                       rmq[power][right - (1 << power) + 1]);
        };
        ...
        for (auto &query : queries) {
            ...
            int leftIndex = upper_bound(
                                segments.begin(),
                                segments.end(),
                                pair<int, int>(left, INF))
                                - segments.begin() - 1;

            int rightIndex = upper_bound(
                                 segments.begin(),
                                 segments.end(),
                                 pair<int, int>(right, INF))
                                 - segments.begin() - 1;
            ...
            auto getSegmentSize = [&](int i) {
                if (i == leftIndex)
                    return segments[leftIndex].second -
                           (left - segments[leftIndex].first);
                if (i == rightIndex)
                    return right - segments[rightIndex].first + 1;
                return segments[i].second;
            };

            auto calculateNewSections = [&](int i) {
                if (s[segments[i].first] == '1')
                    return -INF;
